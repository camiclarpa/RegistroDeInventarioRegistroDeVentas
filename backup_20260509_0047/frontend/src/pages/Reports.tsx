...
// Helper para arrays seguros (evita .map() sobre null/undefined)
const safeArray = <T,>(data: T[] | null | undefined): T[] => Array.isArray(data) ? data : []
...
export default function Reports() {
  ...
  const handleExport = async () => {
    try {
      switch (activeTab) {
        case 'abc':
          if ((!abcData || !safeArray(abcData).length)) { toast.warning('Sin datos para exportar'); return }
          await exportAbcToExcel(abcData)
          break
        case 'profitability':
          if ((!profitabilityData || !safeArray(profitabilityData).length)) { toast.warning('Sin datos para exportar'); return }
  ...
  const abcChartData = Array.isArray(abcData) && safeArray(abcData).slice(0, 20).map((item) => ({
  ...
  const valuationChart = Array.isArray(valuationData) && safeArray(valuationData).map((v) => ({
    name: v.categoryName,
  ...
  const rotationChart = Array.isArray(rotationData) && safeArray(rotationData).slice(0, 15).map((r) => ({
  ...
  const abcSummary = {
    A: safeArray(abcData).filter((i) => i.class === 'A'),
    B: safeArray(abcData).filter((i) => i.class === 'B'),
    C: safeArray(abcData).filter((i) => i.class === 'C'),
  }
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <CardBody>
              ...
              ) : (!abcChartData || !safeArray(abcChartData).length) ? (
            ...
            <div className="overflow-x-auto max-h-96 overflow-y-auto">
              {loading ? <TableSkeleton rows={8} cols={5} /> : (
                <table className="w-full">
                  ...
                  <tbody>
                    {Array.isArray(abcData) && safeArray(abcData).map((item, idx) => (
            ...
            <div className="overflow-x-auto">
              {loading ? <TableSkeleton rows={8} cols={6} /> : (
                <table className="w-full">
                  ...
                  <tbody>
                    {Array.isArray(valuationData) && safeArray(valuationData).map((v) => (
                    ...
                    {safeArray(valuationData).length > 0 && (
            ...
            <div className="overflow-x-auto max-h-96 overflow-y-auto">
              {loading ? <TableSkeleton rows={8} cols={4} /> : (
                <table className="w-full">
                  ...
                  <tbody>
                    {Array.isArray(rotationData) && safeArray(rotationData).map((r, i) => (
      ...
      {activeTab === 'profitability' && (
        <Card className="overflow-hidden">
          ...
          <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
            {loading ? <TableSkeleton rows={10} cols={6} /> : (!profitabilityData || !safeArray(profitabilityData).length) ? (
              ...
              <table className="w-full">
                ...
                <tbody>
                  {Array.isArray(profitabilityData) && safeArray(profitabilityData).map((item) => (
                    <tr key={item.productId} className="table-row">
                      <td className="table-td font-medium">{item.product?.name}</td>
