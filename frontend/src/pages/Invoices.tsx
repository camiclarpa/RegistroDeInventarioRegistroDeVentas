...
export default function Invoices() {
  ...
  const load = useCallback(async (p = page, status = statusFilter, sd = startDate, ed = endDate, cn = customerNameFilter, ci = customerIdFilter) => {
    setLoading(true)
    try {
      const res = await invoiceService.getInvoices({ page: p, limit: LIMIT, status: status || undefined, startDate: sd || undefined, endDate: ed || undefined, customerName: cn || undefined, customerIdentification: ci || undefined })
      setInvoices(res.data)
      setTotal(res.total)
      setTotalPages(res.totalPages)
    } catch { /* handled */ } finally { setLoading(false) }
  }, [statusFilter, startDate, endDate, customerNameFilter, customerIdFilter]) // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => { load() }, [statusFilter, startDate, endDate, customerNameFilter, customerIdFilter]) // eslint-disable-line react-hooks/exhaustive-deps

  const handleFilter = () => { setPage(1); load(1, statusFilter, startDate, endDate, customerNameFilter, customerIdFilter) }
  ...
  const statusVariant = (s: string): 'blue' | 'red' | 'green' | 'gray' => {
    ...
    <div>
      ...
      <Card className="mb-4 p-4">
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <div>
            ...
            <input type="text" className="input-field w-48" value={customerIdFilter} onChange={(e) => setCustomerIdFilter(e.target.value)} placeholder="Buscar por cédula" />
          </div>
          <div>
            <label className="label">Nombre del Cliente</label>
            <input type="text" className="input-field w-56" value={customerNameFilter} onChange={(e) => setCustomerNameFilter(e.target.value)} placeholder="Buscar por nombre" />
          </div>
      ...
      <Card className="overflow-hidden">
                ...
                {invoices.map((inv) => {
                  ...
                  return (
                    <tr key={inv.id} className="table-row">
                      ...
                      <td className="table-td">{inv.customer?.name || inv.customerName || 'Consumidor Final'}</td>
                      ...
                      <td className="table-td">
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            onClick={() => window.open(`/print-ticket-invoice/${inv.id}`, "_blank", "width=400,height=700")}
        ...
        {!loading && invoices.length > 0 && (
          <Pagination page={page} totalPages={totalPages} total={total} limit={LIMIT} onPageChange={(p) => { setPage(p); load(p, statusFilter, startDate, endDate, customerNameFilter, customerIdFilter) }} />
        )}
      </Card>
        ...
        {selectedInvoice && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><span className="text-gray-500">Cliente:</span> {selectedInvoice.customer?.name || selectedInvoice.customerName || 'Consumidor Final'}</div>
