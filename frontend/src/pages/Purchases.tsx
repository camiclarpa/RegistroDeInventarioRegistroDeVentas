...
import { Plus, Trash2, ShoppingBag, CheckCircle, XCircle, Eye, Download, Pencil, FileText, Package } from 'lucide-react'
...
import { VoiceAssistant, type ProductVoiceData } from '@/components/VoiceAssistant'
...
export default function Purchases() {
  ...
  const [batchScannerModal, setBatchScannerModal] = useState(false)
  ...
  const orderScannerRef = useRef<SmartScannerHandle>(null)
  ...
  const handleProductSearch = async (q: string) => {
    ...
    // Buscar en la lista local primero (búsqueda por nombre/SKU)
  ...
  const handleProductScanned = async (product: Product) => {
    // Esta función se llama cuando el escáner detecta un producto
  ...
  const startVoiceAgent = () => {
    if (!wsRef.current || wsRef.current.readyState === WebSocket.CLOSED) {
      ...
      wsRef.current.onmessage = (event) => {
        // Recibir audio del agente y reproducir
  ...
  const handleVoiceProductData = (data: ProductVoiceData) => {
    // Agregar producto con datos de voz al array de items
    const newItem = {
      productId: '',
      quantity: data.quantity || 1,
      costPrice: Math.round((data.salePrice || 0) / 1.3),
      barcode: '',
      voiceData: data,
    }
    append(newItem as any)
    toast.success(`Producto agregado por voz: ${data.name}`)
  }
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            ...
            <button className="btn-primary btn-sm bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700" onClick={() => setBatchScannerModal(true)}>
              <Package className="w-4 h-4" /> Escanear Productos
            </button>
          </div>
        }
      />
        ...
        <form id="order-form" onSubmit={handleSubmit(onSaveOrder)} className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              💡 Escanea el código de barras o busca por nombre/SKU
            </p>
          </div>
          {/* Items */}
          {fields.length > 0 && (
  ...
  const stopVoiceAgent = () => {
    ...
    return (
      ...
      {/* Batch Scanner Modal */}
      <BatchScannerModal
        isOpen={batchScannerModal}
        onClose={() => setBatchScannerModal(false)}
        scannedProducts={[]}
        onProductsAdded={async (newProducts) => {
          try {
            for (const product of newProducts) {
              // Crear producto en inventario
              // Buscar o crear categoría por nombre
              let categoryId = '00000000-0000-0000-0000-000000000001' // Default
              if (product.category) {
                try {
                  const categories = await inventoryService.getAllCategories()
                  const existing = categories.find((c: any) => c.name.toLowerCase() === product.category.toLowerCase())
                  if (existing) {
                    categoryId = existing.id
                  } else {
                    const newCat = await inventoryService.createCategory({ name: product.category, codePrefix: product.category.substring(0, 3).toUpperCase(), marginPercentage: 0 })
                    categoryId = newCat.id
                  }
                } catch (err) {
                  console.warn('⚠️ No se pudo crear categoría, usando default')
                }
              }
              ...
              if (product.brand) {
                try {
                  ...
                  const existing = brands.find((b: any) => b.name.toLowerCase() === product.brand.toLowerCase())
                  if (existing) {
                    brandId = existing.id
                  } else {
                    const newBrand = await inventoryService.createBrand({ name: product.brand })
                    brandId = newBrand.id
                  }
                } catch (err) {
                  console.warn('⚠️ No se pudo crear marca')
                }
              }
              ...
              const payload = {
                ...
                costPrice: parseFloat(product.sale_price) || 0,
              ...
              console.log(`✅ Producto ${product.barcode} creado en inventario`)
            }
            toast.success(`${newProducts.length} producto(s) registrado(s) en inventario`)
          } catch (error) {
            console.error('❌ Error creando productos:', error)
            toast.error('Error al registrar productos en inventario')
          }
        }}
      />
  ...
  const stopVoiceAgent = () => {
    ...
    return (
      ...
      <VoiceAssistant 
        ...
        onProductData={handleVoiceProductData}
      />
