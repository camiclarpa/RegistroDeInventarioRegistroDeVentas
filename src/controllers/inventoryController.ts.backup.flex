...
export async function getBrandById(req: Request, res: Response) {
  try {
    const id = extractId(req.params['id'] ?? req.params['productId']!);
...
export async function getProductById(req: Request, res: Response) {
  try {
    const id = extractId(req.params['id'] ?? req.params['id'] ?? req.params['productId']!);
...
export async function updateProduct(req: Request, res: Response) {
  try {
    ...
    // Helper para asegurar tipos numéricos
    ...
    // Limpiar tipos antes de validar (Prisma requiere números estrictos)
    if (mapped.costPriceAvg) mapped.costPriceAvg = Number(mapped.costPriceAvg);
    if (mapped.salePriceBase) mapped.salePriceBase = Number(mapped.salePriceBase);
    if (mapped.taxRate) mapped.taxRate = Number(mapped.taxRate);
    if (mapped.stockQuantity) mapped.stockQuantity = Number(mapped.stockQuantity);
    if (mapped.minStockLevel) mapped.minStockLevel = Number(mapped.minStockLevel);
    if (mapped.maxStockLevel) mapped.maxStockLevel = Number(mapped.maxStockLevel);
    if (mapped.warrantyDays) mapped.warrantyDays = Number(mapped.warrantyDays);
...
export async function getProductMovements(req: Request, res: Response) {
  ...
  try {
    const productId = extractId(req.params['id'] ?? req.params['productId']!);
...
export async function getInventoryAuditLogs(req: Request, res: Response) {
  try {
    const productId = extractId(req.params['id'] ?? req.params['productId'] ?? req.query['productId'] as string);
...
export async function generateProductCode(req: Request, res: Response) {
  try {
    ...
    if (!categoryId) {
      return fail(res, 'La categoría es requerida para generar el SKU', 400);
    }

    // 1. Obtener prefijo de la categoría
    const category = await prisma.category.findUnique({
      where: { id: categoryId },
      select: { codePrefix: true, name: true }
    });

    if (!category) {
      return fail(res, 'Categoría no encontrada', 404);
    }

    // Obtener código de 3 letras de la categoría
    const catCode = category.codePrefix 
      ? category.codePrefix.substring(0, 3).toUpperCase()
      : category.name.substring(0, 3).toUpperCase();

    // 2. Obtener código de la marca (si existe)
    let brandCode = 'GEN';
    if (brandId) {
    ...
    // 3. Buscar el último correlativo usado para esta categoría
    const lastProduct = await prisma.product.findFirst({
      where: { categoryId },
      orderBy: { createdAt: 'desc' },
      select: { skuInternal: true }
    });

    let nextNum = 1;
    if (lastProduct && lastProduct.skuInternal) {
      // Extraer el número del último SKU (ej: de "ACE-MOT-005" extraer 5)
      const match = lastProduct.skuInternal.match(/(\d+)$/);
      if (match) {
        nextNum = parseInt(match[0], 10) + 1;
      }
    }

    // 4. Construir SKU: CAT-MARCA-001
    const skuInternal = `${catCode}-${brandCode}-${String(nextNum).padStart(3, '0')}`;

    // 5. Generar código de barras EAN-13 numérico basado en el SKU
    // Convertimos caracteres a números para crear un código único
    const barcodeNumeric = skuToEAN13(skuInternal);

    return ok(res, {
      skuInternal,
      barcodeExternal: barcodeNumeric,
      message: 'Códigos generados automáticamente'
    });

  } catch (error) {
    logger.error('[generateProductCode]', error);
    return fail(res, 'Error generando códigos', 500);
  }
}

// Helper: Convierte SKU alfanumérico a EAN-13 numérico
function skuToEAN13(sku: string): string {
  // Remover caracteres no alfanuméricos
  const clean = sku.replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
  
  // Convertir cada carácter a su código ASCII numérico
  let numeric = '';
  for (let i = 0; i < clean.length && numeric.length < 12; i++) {
    numeric += clean.charCodeAt(i).toString().padStart(2, '0');
  }
  
  // Tomar primeros 12 dígitos
  numeric = numeric.substring(0, 12).padEnd(12, '0');
  
  // Calcular dígito verificador EAN-13
  let sum = 0;
  for (let i = 0; i < 12; i++) {
    sum += parseInt(numeric[i]) * (i % 2 === 0 ? 1 : 3);
  }
