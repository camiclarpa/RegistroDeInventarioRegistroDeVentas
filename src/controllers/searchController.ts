...
export async function searchProducts(req: Request, res: Response): Promise<Response> {
  try {
    ...
    const exactByBarcode = await prisma.products.findFirst({
      where: { barcodeExternal: query, isActive: true },
      include: { categories: { select: { id: true, name: true } }, brands: { select: { id: true, name: true } } },
    })
