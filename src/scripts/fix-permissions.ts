...
async function main() {
  ...
  for (const role of roles) {
    const expected = (PERMISSIONS as any)[role.name] ?? [];
    const actual = role.role_permissions.map((rp) => rp.permissions?.name);
    const missing = expected.filter((p: string) => !actual.includes(p));
